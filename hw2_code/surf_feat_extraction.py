...
def get_surf_features_from_video(downsampled_video_filename, surf_function, surf_feat_video_filename, keyframe_interval):
    ...
    frame_generator = get_keyframes(downsampled_video_filename, keyframe_interval)
    
    surf_features = []
    for frame in frame_generator:
        _, descriptors = surf_function.detectAndCompute(frame, None)
        
        if descriptors is None:
            descriptors = np.zeros((25, 128))
        else:
            if descriptors.shape[0] < 25:
                descriptors = [descriptors, np.zeros((25 - descriptors.shape[0], 128))]
                descriptors = np.concatenate(descriptors, axis=0)
            else:
                descriptors = descriptors[:25]
        
        assert descriptors.shape = (25, 128)
        surf_features.append(descriptors)
        
    surf_features = np.concatenate(surf_features, axis=0)
    np.save(sur_feat_video_filename, surf_features)
...
if __name__ == '__main__':
    ...
    for line in fread.readlines():
        ...
        get_surf_features_from_video(downsampled_video_filename, surf,
